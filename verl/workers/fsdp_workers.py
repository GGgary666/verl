...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_teacher_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            outputs = self.actor.compute_teacher_log_prob(data=data)
            output = DataProto.from_dict(
                tensors={"old_bf16_log_probs": outputs["log_probs"]},
                meta_info={"temperature": self.config.rollout.temperature},
            )
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_teacher_log_prob", logger=logger)
